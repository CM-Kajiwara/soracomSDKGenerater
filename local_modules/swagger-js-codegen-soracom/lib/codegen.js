...
var isSoracomSecure = function(path){
    var isMatch = false;
    var notSetTokenPaths = ['/auth','/auth/password_reset_token/issue'];
    notSetTokenPaths.forEach(function(target){
...
var getViewForSwagger2 = function(opts, type){
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            var soracomSecureFlag = isSoracomSecure(path);
            var method = {
                path: path,
                className: opts.className,
                ...
                parameters: [],
                isSoracomSecure : isSoracomSecure(path)            
            };
            var params = [];
            ...
            _.chain(params).forEach(function(parameter) {
                ...
                // if (parameter.schema &&  _.isString(parameter.schema.$ref)) {
                    // var segments = parameter.schema.$ref.split('/');
                    // parameter = swagger.definitions[segments.length === 1 ? segments[0] : segments[2] ];
                // }
                ...
                if(parameter.in === 'body'){
                    if(!parameter.camelCaseName){
                        parameter.camelCaseName = 'body';
                    }
